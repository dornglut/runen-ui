...
fn disabled_button_preserves_action_and_can_be_reenabled() {
    let element = button("Submit")
        ...
        .enabled(true)
        .into_element();
    let (_, _, _, _, _, _, _, _, mut widget, _) = element.into_runtime_parts().into_parts();
...
fn nested_mapping_preserves_non_clone_action_and_state_identity() {
    ...
    let original = button("Submit")
        ...
        .into_element();
    let original_type = original.into_runtime_parts().into_parts().8.state_type_id();
    let mapped = button("Submit")
        .on_activate(|| Child::Submit)
        .into_element()
        .map_action(Parent::Child);
    let (_, _, _, _, _, _, _, _, mut widget, _) = mapped.into_runtime_parts().into_parts();

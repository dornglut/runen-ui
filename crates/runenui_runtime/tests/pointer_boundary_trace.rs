...
use runenui_runtime::{
    ...
    TraceEventFamily, TracePointerContext, TracePointerPath, TracePointerRecordRole, TraceRecord,
...
fn assert_plan_identity(bundle: &TraceRecord, pointer_id: PointerId) {
    ...
    assert_eq!(
        bundle.context().pointer_record_role(),
        Some(TracePointerRecordRole::BoundaryPlan)
    );
...
fn initial_enter_reconstructs_empty_previous_and_exact_current_path() {
    ...
    assert_eq!(
        physical.context().pointer_record_role(),
        Some(TracePointerRecordRole::Observation)
    );
    assert_eq!(
        resolved.context().pointer_record_role(),
...
#[test]
fn callback_failure_never_claims_boundary_delivery() {
    let mut harness = harness();
    harness.runtime.__fail_routed_callback_bridge_for_test();
    let event = pointer_event(&harness, 3, PointerPhase::Down, harness.inside);
    let submission = harness
        .runtime
        .submit_pointer(event)
        .unwrap_or_else(|_| unreachable!("pointer submission is accepted before processing"));
    let sequence = submission.sequence();

    let _ = harness.runtime.pump(PumpBudget::new(
        usize::MAX,
        usize::MAX,
        usize::MAX,
        usize::MAX,
        ...
        record.work_sequence() == Some(sequence)
            && matches!(
                ...
                TraceRecordKind::RoutedIntegrityFailed {
                    failure: TraceRoutedIntegrityFailure::CallbackBridgeFailure,
                }
            )
    }));
    assert!(!records.iter().any(|record| {

...
use runenui_core::{
    Element, NoHostProtocol, UiApp, View, Widget, WidgetMountContext, WidgetUpdateContext, text,
};
use runenui_runtime::{
    AppRuntime, PumpBudget, PumpOutcome, RuntimeConfig, RuntimeStatus, RuntimeTerminalReason,
...
impl Widget<TransactionAction> for MountedValueObserver {
    ...
    fn mount(&self, (): &mut Self::State, _: &mut WidgetMountContext<TransactionAction>) {
        self.mounted_value.set(self.value);
    }

    fn update(&self, (): &mut Self::State, _: &mut WidgetUpdateContext<TransactionAction>) {
        self.mounted_value.set(self.value);
    }
}
...
fn capacity_zero_mounts_state_but_rejects_the_atomic_initial_scheduler_plan() {
    ...
    assert_eq!(
        error.kind(),
        SubmitActionErrorKind::Terminal(RuntimeTerminalReason::Poisoned)
    );
...
fn error_debugging_does_not_require_action_debug() {
    ...
    assert_eq!(
        format!("{error:?}"),
        "SubmitActionError { kind: Terminal(Poisoned), .. }"
    );
    assert!(matches!(error, SubmitActionError::Terminal { .. }));
}

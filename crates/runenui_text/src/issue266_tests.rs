...
fn assert_candidate_matches_oracle(label: &str, map: &TextCaretMap) {
    ...
    assert_eq!(
        ...
        "corrected Parley must not reject a retained RunenUI grapheme edge for {label}"
    );
    assert!(
        exhaustive_validations >= candidate.len(),
        "the exhaustive oracle must perform at least one cursor validation per legal offset for {label}"
    );
}

#[test]
fn grapheme_candidate_matches_exhaustive_oracle_across_controlled_corpus()
...
#[test]
fn grapheme_candidate_matches_exhaustive_oracle_for_preedit_projection()
-> Result<(), Box<dyn Error>> {
...
#[test]
#[ignore = "opt-in issue 266 post-Parley release profile; run with --ignored --nocapture"]

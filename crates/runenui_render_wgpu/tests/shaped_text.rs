...
use runenui_render_wgpu::{
    BackendSelection, PublicationRenderError, PublicationUpdateMode, Renderer, RendererInitError,
    RendererOptions, ResourceCacheOutcome, ResourcePayload, ResourceProvider,
...
-> Result<(), Box<dyn Error>> {
    ...
    assert!(observation.render_succeeded());
    assert!(observation.readback_succeeded());
    assert!(!observation.presented());
...
#[test]
fn shaped_provider_failure_is_reported_before_target_mutation() -> Result<(), Box<dyn Error>> {
    let Some(mut renderer) = renderer_or_adapterless()? else {
        return Ok(());
    };
    let resource = ResourceRef::new(ResourceKind::ShapedTextRun);
    ...
    assert_eq!(first.readback().rgba8_srgb()[..4], [0, 0, 0, 0]);
    Ok(())
}
...
fn shaped_text_matches_checked_in_golden() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(golden.width(), output.readback().extent().width());
    assert_eq!(golden.height(), output.readback().extent().height());
    assert_eq!(
        golden.as_raw(),
        output.readback().rgba8_srgb(),
        "the checked-in golden must come from the same real-wgpu/resource path"
    );
    Ok(())
}

...
M7 has accepted the first real wgpu renderer/resource edge, standalone winit host/native-input/presentation path, AccessKit semantic adapter, and second-consumer pressure for a bounded reusable winit adapter edge. M7 remains incomplete until the native Counter application showcase is accepted and M7D external-host closure is complete. Current maturity is summarized in [status](../status.md). Durable future sequencing belongs in the [roadmap](../roadmap.md). Permanent observable/proof requirements live under [conformance](../conformance/README.md).

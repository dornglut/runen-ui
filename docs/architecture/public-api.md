...
- application-authored text document/revision snapshots plus checked UTF-8 positions, ranges, directional selections, affinities, transient preedit positions, and checked host UTF-16 conversion values without live editing-session authority;
- accepted application-owned transactional-editing values: editable contributions, opaque runtime-issued session/request identities, edit intents, projected selections, inverse/grouping hints, accepted/rejected/transformed immediate resolutions, and editable semantic/action vocabulary;
...
- immutable document/revision-scoped caret maps over that exact retained layout, including grapheme/shaping-valid stops, clip/transform-aware hit mapping, caret/selection/candidate geometry, logical/visual/word/line navigation, and transient durable-versus-synthetic preedit projection mapping;
...
Parley/Fontique/HarfRust/Skrifa/ICU types and the `unicode-segmentation` crate are implementation details and do not become RunenUI API authority. `runenui_text` must not own mounted/runtime/publication identity or scheduling, general layout topology, native/semantic state, application documents, editing sessions/transactions, undo history, framework services, or renderer/GPU/SDF-MSDF atlas state.
...
- bounded owner-local editing-session, selection, preedit and provisional predecessor-chain authority; private edit-origin envelopes; exact immediate post-update resolution/rebuild verification; retired-chain drain; editable semantic publication/action admission; and fail-closed sensitivity/redaction;
...
Runtime must not own application document contents, validation, persistence, or durable undo history; depend on testing convenience, concrete native platforms, concrete renderer implementations, product state, external resource-provider/payload/cache ownership, font/shaping/line-breaking algorithm authority, SDF/MSDF realization, Lyon/backend tessellation, renderer shadow-mask/raster authority; or create a second action/edit/service/interaction/style/semantic/layout/motion/paint/hit/testing authority. Taffy types, node identity, topology, and caches remain private derived implementation state rather than public or retained framework authority.
...
M10B derives caret, hit, selection, navigation, and candidate geometry from that exact retained private layout rather than reconstructing it from public cluster ranges or creating an editor layout. Core document/revision coordinates remain RunenUI-owned UTF-8 values; scalar checks occur at construction, while the correlated map narrows ordinary caret behavior to grapheme and shaping-valid stops. A transient preedit projection retains exact replacement, relative selection, and composition generation while keeping synthetic offsets distinct from durable document positions. It is not a committed document, mounted editing session, or edit transaction.

### One transactional editing authority

M10C keeps application state authoritative for each document value/revision, validation, persistence, sensitivity, and durable undo history. Widgets contribute the current document plus an opaque `EditIntent` mapper; canonical routed defaults and committed-text/composition ingress may stage one bounded provisional intent into the existing FIFO, but only `UiApp::update` may mutate the application document and return the matching immediate `EditResolution`. Runtime privately retains exact ordinary/edit envelope origin, mounted/session generation, predecessor chain, selection/preedit state, and minimum retired draining facts. It verifies the rebuilt contribution before committing effects or services; inconsistent post-mutation results poison rather than guessing rollback. Editable semantics and AccessKit projection reuse the existing semantic identity/publication/action and retained caret-map authorities. Secret content remains absent from semantic values, debug output, trace, replay, diagnostics, and unavailable copy/cut defaults. No runtime editor document, global undo journal, second action queue, or native text model exists.
...
- integrated public-widget/native/real-wgpu closure (M10F);

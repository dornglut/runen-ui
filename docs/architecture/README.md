...
RunenUI currently provides a deterministic headless framework foundation with the complete accepted M6 renderer-neutral paint/hit protocol and the accepted M7A reusable wgpu renderer/resource edge: retained publication, canonical immutable paint/hit products, composition/resource-reference/renderer-metadata/capability semantics, independent deterministic consumer proof, migration closure, independent semantic publication, public deterministic testing, real offscreen/readback and golden evidence, provider/cache realization, and renderer observations. M7B/C/D remain target work for native host/input, accessibility, and external-host closure. See [status](../status.md) for current maturity and the [roadmap](../roadmap.md) for durable sequencing.

Concrete native hosts, accessibility adapters, product state, and engine/ECS integration remain edge or consumer responsibilities. The accepted wgpu renderer/resource edge remains downstream of ordinary public core/runtime contracts; RunenUI core/runtime must not acquire native or backend dependencies merely because future production profiles need adapters for them.

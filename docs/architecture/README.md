...
RunenUI provides a deterministic headless framework foundation with the complete accepted M6 renderer-neutral paint/hit protocol, the complete accepted M7 reference production spine at proof maturity, the accepted M8A production style mechanism at partial styling maturity, the accepted M8B production logical-text/SDF-MSDF mechanism at partial text maturity, and the accepted M8C production runtime layout/text-feedback mechanism at partial layout maturity. M8B provides explicit deterministic/production font-source policy, international shaping and line breaking behind RunenUI-owned contracts, immutable logical artifacts shared by measurement and paint, retained scale-independent shaped-resource bindings, and renderer-owned per-glyph SDF/MSDF realization without shaped-text provider or alpha fallback. M8C uses private low-level Taffy Block/Flex/Grid algorithms over the mounted topology, normalized sizing/positioning/overflow semantics, bounded downstream custom measurement, and final-layout-correlated text state while preserving one runtime-owned final geometry authority. M8D owns integrated responsive/text-heavy closure across those accepted foundations. See [status](../status.md) for accepted maturity and the [roadmap](../roadmap.md) for durable sequencing.

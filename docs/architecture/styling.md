...
M8D's accepted integration evidence exercises resolved style through the same production surface path as responsive layout/text measurement, exact retained shaped-resource paint, semantic bounds/content, and real-wgpu composition. M9A extends that path with runtime-owned common decoration/static composition; M9B extends it again with deterministic staged sampling and exact downstream invalidation. Those additions do not move style/motion resolution into layout, text, semantics, or renderer ownership.
...
M9B deterministic transition/timeline and reduced-motion behavior is accepted current behavior. Remaining M9 work is M9C integrated visual-motion closure, including the final cross-product public/headless, interaction, transform-correlation, real-wgpu motion, and authority-cleanup obligations. External theme serialization/loading, arbitrary renderer material/shader/filter policy, and broader later property families also remain outside the current style authority. The current theme is an explicit host-neutral value supplied in `StyleEnvironment`; no ambient global theme/provider authority exists.

M9A static visual breadth and M9B deterministic motion are current behavior; they must not be described as future merely because M9C remains incomplete. Later property breadth must preserve the same explicit style/motion ownership, staged sampling, and invalidation model rather than creating a parallel cascade or animation engine.
...
M8/M9A/M9B do not justify a `runenui_style` or `runenui_animation` crate. A dedicated crate requires a real independent ownership, dependency, optionality, serialized-source, external-loading, or multiple-consumer boundary that Cargo should enforce; file size or property growth alone is insufficient.

...
| SCENE-META-01 | Paint-publication metadata exposes positive finite raster scale while all scene/hit/pointer geometry remains logical; scale is not scene-coordinate authority and no physical-pixel/DPI/native-window type leaks into neutral core/runtime input. | Scale 1.0/2.0 consumer comparison, unchanged-scene reuse, and logical hit invariance proof | Non-finite/non-positive scale, physical-type API leak, and scene-geometry rescaling proof | Paint-publication metadata diagnostics | M6C | blocked | Required |
| SCENE-META-02 | Damage is deterministic publication-relative logical metadata: it soundly covers renderer-relevant changes relative to the previous accepted paint publication, allows conservative full-surface damage, permits empty damage only when scene plus other renderer metadata are unchanged, and does not change `PaintScene` content identity. | Different-predecessor/same-scene, clean, narrow, and full-damage deterministic corpus | Underdamage, false-empty, history-dependent scene equality, and damage-in-content proof | Damage/publication diagnostics | M6C | blocked | Required |

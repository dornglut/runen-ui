...
| SCENE-OWN-03 | `SurfacePublication` exposes exact immutable paint scene, aligned paint-publication metadata, and hit scene committed with its layout/semantic/diagnostic siblings; consuming one product transfers no live runtime mutation authority. | Public publication access/consumption tests | Public scene/metadata mutators and detached-live-authority proof | Publication identity/alignment diagnostics | M6B | blocked | Required |
| SCENE-OWN-04 | Paint scene data contains no semantic role/action/state tree, `WidgetTypeId`, concrete widget/control kind, mutable mounted state, backend handle, native host type, or predecessor-relative damage history. | Genuine neutral scene consumer/API audit | Forbidden-type/history field compile/API audit and source scan | Repository scene-vocabulary audit | M6B | blocked | Required |
...
| HIT-08 | Hit order is exactly stable ascending `(layer, mounted logical preorder, contribution-local order)` with topmost defined by reverse traversal; `PointerPolicy::{Target, Block, PassThrough}` has exact first-containing semantics, and physical hit policy is not implicitly derived from semantic disabled/hidden/inert state. | Overlap/order/policy/visibility plus disabled-control routing corpus | Alternative tie-break, semantic-state coupling, blocked-through-target, first/last ambiguity, and storage-order dependence proof | Pointer resolution/default trace plus hit policy/order diagnostics | M6C | blocked | Required |
| SCENE-RES-01 | Resource references have explicit neutral kind and provider-scoped stable-by-value logical identity across scene snapshots and are distinct from mounted/semantic/surface identity, bytes, provider objects, scene-local indices, and backend handles. | Resource identity/kind public API and fixture-provider proof | Identity substitution, scene-local reindex, raw bytes/backend handle, and kind-confusion compile/API proof | Resource identity diagnostics | M6C | blocked | Required |
...
| SCENE-META-03 | Scene requirements derive from canonical `PaintScene` content and are aligned as paint-publication metadata; external neutral consumer capabilities are checked without backend-specific scene rewriting or semantic fallback. | Capability acceptance/rejection plus unchanged-scene/different-consumer fixture proof | Alternate-backend scene, capability-in-scene identity, silent lowering, and widget-kind fallback proof | Unsupported-requirement diagnostics | M6C | blocked | Required |

...
AccessKit actions for blur, collapse/expand, custom actions, tooltips, production text replacement/value editing, selection mutation, scrolling, numeric increment/decrement, or other semantics without an exact accepted M5 `SemanticAction` counterpart are not advertised as supported. If such a request is nevertheless delivered, the adapter follows AccessKit's unsupported-action contract: it performs no RunenUI semantic action and does not enter the canonical RunenUI queue. Adapter-local diagnostics may record that boundary decision, but no compatibility action, fallback mapping, or second behavior path is created. In particular, AccessKit scroll actions do not resurrect retired `SemanticAction::LogicalScroll`; semantic scrolling remains later-milestone work while routed M4 `SemanticCommand::LogicalScroll` remains ordinary non-semantic command behavior.

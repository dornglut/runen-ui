...
| Initial work ordering | Initial effects append in collector order before subscription starts append in declaration order | Runtime integration test | Required for M4 |
...
| Application state change | Application subscription declaration is reevaluated after every successful action/reconciliation | Runtime integration test | Required for M4 |

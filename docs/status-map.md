...
| Layout and measurement | `proof` | Explicit normalized constraints; borrowed measurement provider; computed padding; one measurement per node; finite saturating row/column arrangement and bounds; overflow diagnostics | Narrow algorithm; no sizing vocabulary, alignment, flex/grid, wrapping, overlays, clipping, scrolling, baselines, or incremental layout | M7–M8 |
...
M0 and M1 are complete. M2 is queued; M3 remains blocked by M2.

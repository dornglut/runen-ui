...
| Application model | `proof` | Core-owned `UiApp`; application state/actions; ordered initial/update effects; state-derived subscriptions; typed host protocol; action/command/pointer submission through one generalized FIFO | One mounted root; keyboard/text event families remain blocked | M4B/M4C1 complete; M4C2/M4C3 owner-accepted; M4C4 proof-complete |
...
| Persistent widget state and lifecycle | `usable` | State-aware lifecycle, activation, routed-event, exact-generation focus memory, pointer cleanup, and deterministic shutdown capabilities | Text interaction state remains M4C5 | M3/M4B/M4C1 complete; M4C2/M4C3 owner-accepted; M4C4 proof-complete |
| Events and interaction | `proof` | Core-owned semantic-command, pointer, and focus-event protocols; checked downstream C/T/B; atomic focus out/in; propagation/default control; conservative admission; release-inside activation; checked current/historical surface ingress | No native host translation, production scrolling, keyboard routing, text/IME, or resolved automation/accessibility targeting | M4C1/M4C2/M4C3 owner-accepted; M4C4 proof-complete |
| Normalized UI navigation commands | `proof` | Activation/menu commands, pointer-derived logical scroll, linear/directional focus, request/restore, and focus logical-scroll derivation share the canonical FIFO/routing/trace across normalized sources | Production scroll mutation and source-specific target resolution remain later | M4C1 complete; M4C3 owner-accepted; M4C4 proof-complete; M4C5/M5 later |
| Directional/spatial focus navigation | `proof` | Current published rectangles, nearest live scope, current eligibility, exact generation, and mounted-order tie-break satisfy DF-01–DF-20 through public commands | Private score is deliberately not public; multi-surface transfer remains absent | M4C4 proof-complete |
...
| Focusability facts | `proof` | Widgets declare automatic/explicit/hidden focusability and nested scope policy; runtime revalidates live enabled eligibility | This is not the M5 semantic accessibility model | M4C4 proof-complete; M5 later |
...
| Trace and observability | `partial` | One bounded canonical sequence covers scheduler, routed command, surface, pointer, focus selection/policy/restoration/transition/notification/focus-within, and modality facts | No text schema, normalized trace-v2 schema, external sink, JSONL export, redaction, or replay | M4B/M4C1/M4C2/M4C3 owner-accepted; M4C4 proof-complete; M4D blocked |
...
proof-complete on its active branch but is not owner-accepted or merged; M4C5
and M4D1–M4D3 remain blocked in sequence. No keyboard/text/IME, authored automation resolution, semantic accessibility mapping,

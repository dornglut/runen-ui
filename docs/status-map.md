...
| Application model | `proof` | Core-owned `UiApp`; application state/actions; ordered initial/update effects; state-derived subscriptions; typed host protocol; explicit submission and one generalized FIFO | One mounted root; routed event transactions remain M4C | M4B implemented; pending owner acceptance |
| Mounted runtime identity and indexing | `usable` | Runtime-local generational `MountedNodeId`, distinct `SemanticNodeId`, logical-preorder `MountedTreeIndex`, stale/foreign rejection, authored-ID lookup diagnostics | Public runtime-local event identity and its shared namespace remain an M4C1 migration | M3 complete; M4C1 planned |
...
| Persistent widget state and lifecycle | `usable` | State-aware capabilities, preorder mount/update, postorder removal/replacement/shutdown, exact-mounted-owner lifecycle/activation work output, state drop after unmount, interaction slots, idempotent shutdown | Routed event callbacks and interaction state remain M4C | M3 complete; M4B implemented pending acceptance |
| Events and interaction | `proof` | Typed pointer/keyboard vocabulary; mounted hit targets; traversal focus; explicit `WidgetActivationOutput`; exact activation saturation; authoritative state-only/coalesced/empty outcomes | Proof helpers remain direct and physical-policy-specific; there is no routed transaction, shared core event identity, `WidgetEventOutput`, displayed-generation context, pointer identity/capture, release-based activation, focus scopes, text/IME separation, or final event-level diagnostics | M4C0 decision gate, then M4C1–M4C5 |
| Normalized UI navigation commands | `planned` | The M4C routed-command protocol and delivery decisions are frozen in the review-draft [M4C charter](architecture/m4c-delivery-and-routed-transaction-charter.md) | No accepted implementation; M4C1 may not begin before M4B merge and M4C0 owner acceptance | M4C1, M4C3–M4C4 |
| Directional/spatial focus navigation | `absent` | None | Current focus movement is linear traversal only | M4C4 |
| Effects and scheduling | `proof` | One atomic planner; live-only generational producers; before-unmount revocation; `Starting -> Running` send subscriptions; tombstone-free host authority; checked mandatory trace plans; once-claimed serialized wake callbacks outside framework synchronization | Runtime supplies executor/source adapters rather than a default thread pool; routed event output remains M4C | M4B correction implemented; pending owner acceptance |
...
M0–M3 and M4A are complete. ADR 0005 and ADR 0006 remain the accepted M4
behavioral charter. The M4B application-work and deterministic-scheduler
correction is complete at the exact reviewed head and pending owner acceptance.
The review-draft [M4C delivery and routed-transaction charter](architecture/m4c-delivery-and-routed-transaction-charter.md)
closes the remaining implementation decisions but is not owner-accepted by its
presence alone. After M4B is accepted and merged, documentation-only M4C0 must
assign stable conformance IDs, proof ownership, slice ownership, and exact status
across the canonical matrix and current-contract documents. M4C1–M4C5 remain
blocked until that gate passes. M4D1–M4D3 remain blocked pending their preceding
M4C slices. M4 is not complete.

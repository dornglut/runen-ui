...
| Application model | `proof` | Core-owned `UiApp`; application state/actions; ordered initial/update effects; state-derived subscriptions; typed host protocol; action/command/pointer submission through one generalized FIFO | One mounted root; focus-scope/keyboard/text event families remain blocked | M4B/M4C1 complete; M4C2 owner-accepted; M4C3 proof-complete |
...
acceptance authority. M4C3 is proof-complete on the active branch and awaits
owner acceptance and merge; M4C4–M4C5 and M4D1–M4D3 remain blocked in sequence.
No focus-scope/modality, keyboard/text/IME, authored automation resolution, semantic accessibility mapping,

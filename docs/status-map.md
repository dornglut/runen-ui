...
| Application model | `proof` | Core-owned `UiApp`; application state/actions; ordered initial/update effects; state-derived subscriptions; typed host protocol; action and semantic-command submission through one generalized FIFO | One mounted root; surface/pointer/focus-scope/text event families remain blocked | M4B complete; M4C1 proof-complete pending acceptance |
| Mounted runtime identity and indexing | `usable` | One core-owned opaque namespace backs generational `MountedNodeId` and distinct `SemanticNodeId`; logical-preorder `MountedTreeIndex`; foreign/stale/missing distinction; authored-ID diagnostics | Process-local and non-serialized; no M4C2 surface identity yet | M3 complete; M4C1 proof-complete pending acceptance |
...
| Persistent widget state and lifecycle | `usable` | State-aware lifecycle, activation, and routed-event capabilities; preorder mount/update; postorder removal/replacement/shutdown; exact-owner work; state drop after unmount; interaction slots | Later surface/pointer/focus/text state remains M4C2–M4C5 | M3/M4B complete; M4C1 proof-complete pending acceptance |
| Events and interaction | `proof` | Narrow core-owned semantic-command event protocol; checked downstream `Widget::event`; immutable capture/target/bubble route; propagation/default control; ordered mapped output; conservative maximum-safe admission; structured routed-integrity diagnosis; semantic activation | No displayed-generation context, pointer identity/capture/release-inside behavior, focus scopes/modality, keyboard routing, text/IME, or resolved automation/accessibility targeting | M4C1 corrected proof complete pending acceptance; M4C2–M4C5 blocked |
| Normalized UI navigation commands | `proof` | Exact-target `Activate`, `CancelOrBack`, `OpenMenu`, and `OpenContextMenu` share canonical submission/FIFO/routing/trace; programmatic, automation, accessibility-stub, and controller sources converge | Logical scrolling waits for M4C3; focus commands/scopes wait for M4C4; source-specific resolution waits for M4C5/M5 | M4C1 proof-complete pending acceptance; M4C3–M4C5 blocked |
| Directional/spatial focus navigation | `absent` | None | Current focus movement is linear traversal only | M4C4 |
| Effects and scheduling | `proof` | One atomic planner; live-only generational producers; routed exact-owner work output; before-unmount revocation; `Starting -> Running` send subscriptions; tombstone-free host authority; checked mandatory trace plans; once-claimed serialized wake callbacks | Runtime supplies executor/source adapters rather than a default thread pool | M4B complete; M4C1 proof-complete pending acceptance |
...
| Trace and observability | `partial` | One bounded canonical sequence covers scheduler facts plus command acceptance, accepted-work processing rejection, submission-rejection trace absence/non-consumption, routed integrity, route snapshot/phases, control/state/output/default/commit facts, and parentage into actions/delegated commands | No later surface/pointer/focus/text schemas, normalized trace-v2 schema, external sink, JSONL export, redaction, or replay | M4B complete; M4C1 corrected proof complete pending acceptance; M4C2–M4D3 blocked |
...
acceptance authority. M4C1 implementation and corrected proof are complete at
this exact head, pending owner acceptance and merge. M4C2–M4C5 and M4D1–M4D3 remain
blocked in sequence. No surface-generation, pointer-capture, release-inside,
focus-scope/modality, keyboard/text/IME, authored automation resolution,
semantic accessibility mapping, export, sink, or replay maturity is upgraded.
M4 is active and not complete.

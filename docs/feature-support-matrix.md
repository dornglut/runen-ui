...
total / 237 owner-accepted`, and M6 truth is `36 total / 0 owner-accepted / 36
...
| External custom widgets | `supported` | State-aware public widgets, checked routed bridge, non-`Clone` mapping, pointer/focus C/T/B, keyboard/text/composition conformance, canonical semantic contribution/owner-local bounds authoring, accepted semantic publication/action conformance, and genuine downstream M5D public-harness conformance | Production paint/layout, editable text, and native accessibility adapters remain later | M3/M4/M5A–M5D complete |
...
| Pointer hit targeting | `proof` | Current/retained frame rectangle targeting produces generation-safe physical paths kept separate from routed/captured owners; M5D constructs current pointer input from the exact public publication context | No explicit hit scene, stacking, clips, transforms, visibility, or M6 pointer policy | M4/M5D proof; M6 |
| Pointer activation | `proof` | Canonical down/move/up/cancel lifecycle; eligible primary release inside the exact live pressed owner derives one routed `Activate`; Counter and M5D harness prove public physical convergence | No native host translation or broader production control policy | M4/M5D complete; M9/M10 later |
| Keyboard activation | `proof` | Exact focused keyboard ingress routes C/T/B; non-repeated Enter down and matched Space down/up append canonical `Activate`; M5D downstream input tests use the public harness | No native host translation or production control policy | M4/M5D complete |
| Focus traversal | `proof` | Root/nested scopes; current-order next/previous; published-geometry direction; explicit boundary policy; exact restoration; resulting focus changes publish through the semantic sibling without semantic callback re-entry; semantic `RequestFocus` converges through the same default authority; M5D public harness proves controller traversal/restoration | Cross-surface transfer remains M10 | M4/M5B–M5D complete; M10 later |
| Event capture/target/bubble | `proof` | Immutable exact-mounted C/T/B semantic-command, pointer, focus, keyboard, committed-text, and composition routes; checked bridges; independent propagation/default control; accepted M5C semantic requests join the same routed path after private admission | No native event translation | M4/M5C complete; M5D public test coverage |
...
| Text input and IME events | `proof` | Nonempty committed Unicode text and opaque generation-scoped composition start/update/end/cancel route to exact focused opt-in capability; M5D downstream tests submit them through public harness ingress and prove default trace redaction/replay | No editable text, native IME object, selection, or text-layout contract | M4/M5D proof; M8–M10 later |
| Accessibility/programmatic activation | `proof` | Exact-mounted programmatic/accessibility-stub/controller origins converge through `submit_command`; automation resolves exactly one authored ID; M5C resolves exact current `SurfaceId + SemanticNodeId + SemanticAction` through private bindings into the same canonical command/default path; M5D exposes exact scoped semantic-action test helpers | No native accessibility adapter | M4/M5C–M5D complete; M10 later |
...
| Directional/spatial focus navigation | `proof` | Current publication geometry and mounted-order final tie-break satisfy DF-01–DF-20 through public submission; M5D downstream tests consume current public geometry | Private score is intentionally not API | M4/M5D complete |
| Controller activation/cancel/menu commands | `proof` | Normalized controller source submits the same exact-target semantic commands without raw device vocabulary; M5D proves normalized controller focus/command use through public APIs | Raw device mapping/identity/axes remain host/M10 scope | M4/M5D complete; M10 |
...
| Application action and command submission | `proof` | `submit_action` returns exact action recovery; `submit_command` returns `CommandSubmission` or exact owned target/command/origin recovery; `submit_semantic_action` returns the existing `CommandSubmission` or exact owned semantic request with typed surface/identity/support/state/readiness/capacity rejection; M5D provides thin public harness delegates without bypassing canonical ingress | Native adapters remain later | M4/M5C–M5D complete; M10 |
...
| Processed-envelope pump budget | `proof` | Four-argument `PumpBudget`; exact report and outcome; M5D exposes the same explicit bounded pump through `TestHarness` | None in implemented work scope | M4/M5D complete |
...
| Deterministic scheduler testing | `proof` | Public M5D `TestHarness` composes `ManualClock`, explicit finite `SettleBudget`, exact `PumpReport`, future-deadline observation, externally pending host-work observation, and bounded self-requeue without wall-clock sleeps | No property/fuzz framework or production animation harness | M4/M5D complete |
...
| Deterministic headless measurement | `proof` | Unicode-scalar count with fixed metrics; M5D fixed/configurable test surface can inject a public custom measurement provider | Not production text geometry | M5D proof; M8 production |
...
| Paint primitives/resources | `unsupported` | Accepted ADR 0007 defines the minimum target vocabulary | No current shapes, strokes, image/shaped-run resource refs, clips, layers, or damage API/behavior | M6 |
...
| Retained surface-input snapshots | `proof` | Configurable nonzero bounded immutable hit-test snapshots, exact historical targeting, oldest-first retirement, and retired/missing/foreign/revision outcomes | Retains proof-level hit facts only; accepted ADR 0007 requires a production `HitTestScene` without changing current behavior yet | M4 complete; M6 later |
| Multi-surface publication | `unsupported` | None | No independent surface lifecycle or scale | M10 |
| Debug/renderer semantic separation | `proof` | Renderer-facing `SurfaceFrame`, `SurfaceNode`, and debug output do not carry production semantic contribution; semantics are consumed from the sibling `SemanticPublication`, M5C action resolution remains private runtime authority, and M5D inspection consumes both products without joining them | No native accessibility adapter, paint-scene consumer, or renderer backend | M5B–M5D complete; M6/M10 later |
| Deterministic paint-scene consumer | `planned` | Accepted ADR 0007 and the M6 matrix require two independent deterministic consumers | No M6 scene implementation exists yet | M6 |
| Conventional renderer backend | `unsupported` | None | Protocol implementation must stabilize first | M10 |
...
| Headless deterministic metrics | `proof` | Fixed scalar width and line height; M5D may inject custom public intrinsic measurement | Not shaping, grapheme measurement, or font metrics | M5D proof; M8 |
...
| Headless host profile | `partial` | Public M5D `TestHarness` over the deterministic mounted runtime with manual clock, finite settle/pump budgets, configurable fixed surface, accepted synthetic input/semantic ingress, typed host requests/pending-work observation, semantic/layout/hit/paint inspection, canonical trace export, and inert replay | No native host adapter or production renderer/text/control stack | M4/M5A–M5D complete; M10 production host |
...
| Capability | Current support | Current proof or API | Known limitation | Target milestone |
|---|---|---|---|---|
| Workspace unit/integration tests | `supported` | Substantial deterministic proof suite plus public-only downstream custom-widget package including M5A authoring, M5B publication, M5C semantic-action/readiness, and M5D public-harness conformance | Ubuntu-only CI; no production platform matrix | M5D complete; M11 |
...
| Runtime trace | `partial` | One accepted bounded canonical M4D1-normalized graph, M4D2 deterministic JSONL v1 export/redaction/action-label/sink surface, M4D3 inert offline replay, M5C semantic binding/processing-rejection/default-invalidation outcomes in the same schema, and M5D read-only harness export/replay/redaction proof | Export/sink/replay remain headless proof infrastructure rather than a production observability service | M4/M5C–M5D complete |
| Bounded canonical trace retention | `proof` | Configured capacity including zero, oldest-first eviction, non-wrapping `TraceSequence`, borrowed iteration, exclusive dropped-before watermark, normalized-schema proof, deterministic v1 projection, accepted offline causal replay consumption, M5C semantic causal records, and M5D public harness observation | Replay validates the serialized retained causal projection; it does not create live runtime authority | M4/M5C–M5D complete |
...
| Public headless test harness | `proof` | Public downstream `runenui_testing::TestHarness<App>` with deterministic `ManualClock`, configurable nonzero fixed surface, explicit bounded pump, finite `SettleBudget`, public ingress delegates, snapshot-scoped semantic query/target types, read-only state/focus/reconciliation/frame/layout/hit/paint/semantic/trace/replay observations, and genuine Counter/external-widget conformance | No private mutation seams, wall-clock waits, snapshot-golden framework, native host translation, or stable pre-1.0 compatibility promise | M5D complete |
| Semantic/layout/hit/paint assertions | `partial` | M5D exposes typed semantic query/results plus direct public publication/frame/layout/hit/paint inspection and exact semantic-action helpers; ordinary Rust assertions remain the intended assertion surface | No macro-heavy assertion DSL, snapshot-golden framework, or production M6 scene assertions | M5D complete; M6 production scenes |
| Deterministic time/tasks | `proof` | M5D `ManualClock` advancement, future timer observation, finite settle, externally pending host work, and bounded self-requeue prove deterministic scheduler behavior without sleeping | No production animation system | M4/M5D complete |
| Snapshot/golden/replay tests | `partial` | Deterministic JSONL snapshots are byte-stable; M4D3 round-trips real exported JSONL and reconstructs Counter causality; M5B adds deterministic semantic snapshots/update chains; M5C proves semantic trace export/replay remains inert; M5D exposes exact snapshot queries and trace export/replay through the public harness | No snapshot-golden framework or application-specific expectation engine | M4/M5B–M5D complete |
| Property/fuzz testing | `unsupported` | None | Production hardening work | M11 |
| Benchmarks and budgets | `unsupported` | None | No performance gates; #59 owns retained-publication clone investigation | M6A #59; M11 |

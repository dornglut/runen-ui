...
| Granular invalidation | `supported` | Explicit phase functions, exact context key, topology-only whole-surface cache, current mounted common-field reads, independently verified `SurfacePhaseReport`, semantic contribution caching with structural child-count invalidation | Whole-surface structural rebuilds remain conservative; semantic product updates and production incremental layout are later work | M3/M5A complete; M5B/M7/M11 |

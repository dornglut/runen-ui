...
| Application action and command submission | `proof` | `submit_action` returns exact action recovery; `submit_command` returns `CommandSubmission` or exact owned target/command/origin recovery; `submit_semantic_action` returns the existing `CommandSubmission` or exact owned semantic request with typed surface/identity/support/state/readiness/capacity rejection | M5D convenience helpers and native adapters remain later | M4/M5C complete; M5D/M10 |

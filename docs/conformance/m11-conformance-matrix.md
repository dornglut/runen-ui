...
| M11CTRL-02 | Public `Button<Action>` activation converges through the same canonical semantic default and application-action path for semantic/accessibility, pointer, Enter keyboard, automation, programmatic, and normalized controller origins. | `crates/runenui_testing/tests/m5e_counter_origins.rs::counter_converges_semantic_pointer_keyboard_automation_and_programmatic_activation`; `examples/reference_winit/tests/controller_input.rs` | Accepted M4 stale/disabled/non-actionable/prevented activation corpus | Existing M4 command/default/action trace records | M11A | proof-complete | Required |

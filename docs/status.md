...
| Routed interaction | proof | pointer, focus scopes/navigation, keyboard, committed text, composition, automation, semantic commands, plus accepted loss-preserving native winit translation in the reference host | production controls/text remain later; supported platform breadth belongs to M13 |
...
| Concrete renderer backend | proof | accepted reusable wgpu renderer/resource edge with real offscreen pixels, provider/cache realization, exact golden/readback evidence, renderer observations, and real native presentation through the accepted winit host | external-host closure remains M7; broader production/platform profiles remain later |
| Native window/event-loop host | proof | accepted standalone winit host with host-owned wake/pump/redraw driving, displayed-frame input mapping, resize/raster-scale transitions, native keyboard/text/IME/focus translation, real wgpu presentation, and bounded target recovery | native application pressure and external-host closure remain M7; supported platform breadth belongs to M13 |
| Native accessibility adapter | proof | accepted AccessKit adapter over ordinary semantic publication with stable adapter-owned identity, exact delta/full-resync behavior, exact action translation, install-before-show ordering, proxy callbacks, and host-thread runtime ingress | native application reuse remains M7 application pressure; broader platform profiles belong to M13 |
...
M6 is accepted complete at `proof` maturity: retained publication, canonical renderer-neutral paint/hit products, composition/resource/metadata/damage/capability semantics, independent-consumer proof, public testing convergence, and migration closure are accepted on the default branch. M7A, M7B, and M7C are accepted at `proof` maturity: the reusable wgpu renderer/resource edge, real offscreen/readback and golden evidence, standalone winit host/native-input/presentation path, and AccessKit semantic-publication/action adapter are accepted on the default branch. M7 remains incomplete until the bounded native application showcase and M7D external-host/closure work are accepted. Accepted target documents do not promote capability maturity by themselves; maturity changes only when accepted default-branch implementation and required conformance evidence change.

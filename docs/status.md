...
| Concrete renderer backend | proof | accepted reusable wgpu renderer/resource edge with real offscreen pixels, provider/cache realization, exact golden/readback evidence, renderer observations, real native presentation through the accepted winit hosts, and a winit-free downstream host proof using the same runtime/publication/renderer/resource contracts | broader production/platform/device-loss breadth belongs to M13 |

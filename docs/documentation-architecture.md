...
Code and executable tests establish what the current implementation demonstrably does. Accepted ADRs and durable architecture/design documents define required architectural decisions. Permanent conformance documents define required observable behavior and proof obligations. If observed implementation and an accepted contract disagree, the repository has a defect or needs an explicit contract revision; implementation does not silently amend the contract.

GitHub issues and the Engineering Portfolio own live work. Pull requests and exact-head CI own delivery evidence.
...
| `docs/architecture/` | current durable system boundaries, ownership, dependency direction, conceptual public contract | live delivery state or speculative target specification |
| `docs/adr/` | durable accepted decisions, rationale, alternatives, consequences, bounded acceptance provenance where needed | current issue/PR state |
| `docs/design/` | accepted target behavior or migration contracts when independently warranted | speculative taxonomy or implementation ledger |
| `docs/conformance/` | permanent observable requirements, proof obligations, and directly supporting accepted contract material | in-flight branch/PR/review-state database |
...
- current branch or mutable feature-head SHA;
- current pull-request inventory or PR number as workflow state;
- workflow-run identifiers used as current execution state;
- current assignee, blocker, priority, or next-action state;
- copied Project fields;
- speculative branch names or draft PR titles;
- in-flight acceptance checkpoints that belong to a pull request.

Stable provenance may remain in `history/`, `reports/`, ADR rationale, changelog material, or a frozen conformance contract when it is genuinely necessary to explain an accepted decision or proof boundary. Such provenance must not be used to select current work and should not be copied merely because Git/PR history already records it.

## Conformance state

Permanent conformance IDs and proof obligations remain repository authority. A matrix status represents accepted default-branch conformance state only. In-flight implementation, proof, review, and exact-head evidence stay in the owning issue and pull request until acceptance changes repository state.

...
fn audit_durable_current_document(relative: &str, contents: &str, findings: &mut Vec<Finding>) {
    for prefix in [
        ...
        if contents.contains(prefix) {
            findings.push(Finding::fatal(
                ...
                Some(relative.to_owned()),
                format!("durable current authority must not mirror live state via {prefix:?}"),
            ));
        }
    }
    ...
    if contains_full_lowercase_sha(contents) {
        findings.push(Finding::fatal(
            ...
            Some(relative.to_owned()),
            "durable current authority must not embed full commit SHAs",
        ));
    }
    ...
    for marker in [
        ...
        if contents.contains(marker) {
            findings.push(Finding::fatal(
                ...
                Some(relative.to_owned()),
                format!("durable current authority contains volatile marker {marker:?}"),
            ));
        }
    }
}
...
fn contains_full_lowercase_sha(contents: &str) -> bool {
    ...
    (0..=bytes.len() - 40).any(|start| {
        ...
        candidate.iter().copied().all(is_lower_hex)
            && start
                .checked_sub(1)
                .is_none_or(|index| !is_lower_hex(bytes[index]))
            && bytes
                .get(start + 40)
                .copied()
...
mod tests {
    use super::{
        Finding, OutputFormat, Severity, audit_durable_current_document,
        contains_full_lowercase_sha, json_escape, parse_output_format,
    };
    ...
    #[test]
    fn durable_current_authority_rejects_live_issue_link() {
        let mut findings = Vec::new();
        audit_durable_current_document(
            "README.md",
        ...
        assert_eq!(findings[0].code, "authority.volatile_github_state");
    }

    #[test]
    fn durable_current_authority_rejects_commit_sha() {
        let mut findings = Vec::new();
        audit_durable_current_document(
            "docs/status.md",
        ...
        assert_eq!(findings.len(), 1);
        assert_eq!(findings[0].code, "authority.volatile_commit_sha");
    }
